...
window.addEventListener('message', function(event) {
    ...
    if (data.action == 'updateMoney') {
        // Format cash and bank values with commas using .toLocaleString()
    document.getElementById('cash').innerHTML = `$${(data.cash || 0).toLocaleString()}`;
    document.getElementById('bank').innerHTML = `$${(data.bank || 0).toLocaleString()}`;
    }
});
